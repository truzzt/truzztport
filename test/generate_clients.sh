...
for ((i=1;i<=$1;i++)); do
    ...
    connector_name="connector_${i}.dev.truzzt.eu"

    # Creating the connector certificate
    source scripts/setups/setup_client.sh $connector_name "DE" "truzzt" "ids"

    # Extracting the SKI:AKI and private key from the certificate
    TEMP_FILE="$(mktemp)"
    CRT_FILE=data/cert/$connector_name.crt 
    EDC_P12=data/cert/$connector_name.p12
...
tar -cf certs.tar data/cert/*.dev.truzzt.eu